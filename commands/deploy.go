...
var deployCmd = &cobra.Command{
	Use:   "deploy",
	Short: "sub-command deploy, deploys resource using terraform init, plan and apply",
	Long:  `sub-command deploy, deploys resource using terraform init, plan and apply`,
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) != 1 {
			log.Fatal("subcommand -> deploy, takes one argument as tfvars filename")
		}
		err := runDeploy(args)
		if err != nil {
			log.Fatal(err)
...
func runDeploy(args []string) error {
	...
	if strings.Split(splitPath[len(splitPath) - 1], ".")[1] == "tfvars" {
		log.Infof("will run terraform init, plan and apply on [%s]", splitPath[len(splitPath) - 1])
		fmt.Println()
		...
		if err = TerraformApply(backendConfig); err != nil {
			return err
		}
		...
		if err = TerraformDestroy(backendConfig); err != nil {
			return err
		}
	} else {
